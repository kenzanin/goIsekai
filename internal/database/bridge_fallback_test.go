...
func TestGetMangaCached(t *testing.T) {
	...
	mangaID, err := db.UpsertManga(Manga{
		PluginID: "plugin-1", SourceMangaID: "src-100",
	...
	if m.ID == 0 {
		t.Errorf("manga id = %v, want non-zero", m.ID)
	}
	if m.ID != mangaID {
		t.Errorf("manga id = %d, want %d", m.ID, mangaID)
	}
	...
	if m2.ID != 0 {
		t.Errorf("expected empty manga for cache miss, got id=%d", m2.ID)
	}
...
func TestListChaptersCached(t *testing.T) {
	...
	chapters := []Chapter{
		{MangaID: mangaID, SourceChapterID: "ch-1", Title: "Chapter 1", ChapterNum: 1},
		{MangaID: mangaID, SourceChapterID: "ch-3", Title: "Chapter 3", ChapterNum: 3},
		{MangaID: mangaID, SourceChapterID: "ch-2", Title: "Chapter 2", ChapterNum: 2},
	}
	for _, c := range chapters {
		_, err := db.UpsertChapter(c)
		if err != nil {
	...
	cached, err := db.ListChaptersCached(mangaID)
	if err != nil {
...
func TestListChaptersCachedEmpty(t *testing.T) {
	...
	// No chapters stored.
	cached, err := db.ListChaptersCached(0)
	if err != nil {
		t.Fatalf("ListChaptersCached: %v", err)
	}
	if cached == nil {
...
func TestMangaDescriptionIfCustom(t *testing.T) {
	...
	_, err = db.UpsertManga(Manga{
		PluginID: "p1", SourceMangaID: "s1",
...
func TestGetMangaDetailsOfflineFallback(t *testing.T) {
	...
	mangaID, err := db.UpsertManga(Manga{
		PluginID: "test-plugin", SourceMangaID: "src-555",
...
func TestNewSinceField(t *testing.T) {
	...
	// Set new_since via raw SQL (UpsertManga does not populate it).
	_, err = db.db.Exec(
		`UPDATE mangas SET new_since = ? WHERE id = ?`,
		"2026-09-10T12:00:00Z", mangaID,
	)
	if err != nil {
		t.Fatalf("update new_since: %v", err)
	}

	// Verify new_since is stored.
	var ns string
	err = db.db.QueryRow(`SELECT new_since FROM mangas WHERE id = ?`, mangaID).Scan(&ns)
	if err != nil {

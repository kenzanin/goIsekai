...
return function(data)
    ...
    emit('    <h1 class="text-xl font-semibold">Logs <span class="text-xs font-normal text-neutral-500">last ' .. h(tostring(limit)) .. ' lines · ring buffer 2000</span></h1>')
    emit('    <div class="flex items-center gap-2">')
    emit('        <span id="log-paused" class="hidden text-xs px-2 py-1 rounded-md bg-amber-500/15 text-amber-400">Paused — selecting text</span>')
    emit('        <select id="log-filter" class="bg-neutral-900 border border-neutral-700 rounded-md px-2 py-1.5 text-xs">')
    ...
    emit('        </select>')
    emit('        <select id="log-limit" class="bg-neutral-900 border border-neutral-700 rounded-md px-2 py-1.5 text-xs">')
    for _, n in ipairs(limits) do
        local sel = n == limit and " selected" or ""
        emit('            <option value="' .. h(tostring(n)) .. '"' .. sel .. '>' .. tostring(n) .. ' lines</option>')
    end
    emit('        </select>')
    emit('        <button id="log-copy" type="button" class="border border-neutral-700 hover:bg-neutral-800 rounded-md px-3 py-1.5 text-xs font-medium">Copy</button>')
    emit('    </div>')
    emit('</div>')

    emit('<div id="logview" class="bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2 mb-6 overflow-y-auto max-h-[70vh] font-mono text-xs">')
    for i, line in ipairs(logs) do
        emit('    <pre class="text-xs font-mono whitespace-pre-wrap break-all border-b border-neutral-900 py-0.5 select-text">' .. h(line) .. '</pre>')
    end
    emit('</div>')
    ...
    emit('      pre.className = "text-xs font-mono whitespace-pre-wrap break-all border-b border-neutral-900 py-0.5 select-text";')

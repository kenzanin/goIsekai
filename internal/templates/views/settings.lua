...
return function(data)
	...
	emit(
		'<p class="text-xs text-neutral-500 mb-6">Config: <code class="text-xs bg-neutral-800 rounded px-1.5 py-0.5">'
			.. h(path)
			.. "</code></p>"
	)
	...
	if cfg then
		...
		emit(
			'                    <option value="debug"'
				.. (logLevel == "debug" and " selected" or "")
				.. ">debug</option>"
		)
		emit(
			'                    <option value="info"' .. (logLevel == "info" and " selected" or "") .. ">info</option>"
		)
		emit(
			'                    <option value="warning"'
				.. (logLevel == "warning" and " selected" or "")
				.. ">warning</option>"
		)
		emit("                </select>")
		emit("            </div>")
		...
		emit(
			'                <input id="user_agent" name="user_agent" type="text" value="'
				.. h(cfg.UserAgent or "")
				.. '" class="w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2 text-sm">'
		)
		emit("            </div>")
		...
		emit(
			'                <input id="accept_language" name="accept_language" type="text" value="'
				.. h(cfg.AcceptLanguage or "")
				.. '" class="w-full bg-neutral-900 border border-neutral-700 rounded-md px-3 py-2 text-sm">'
		)
		emit("            </div>")
		emit("            <div>")

	...
	if #mangas > 0 and q == "" then
		...
		local function statCard(value, label, color, longText)
			...
			if longText then
				...
				cls = color and (' class="text-lg font-semibold ' .. h(color) .. '"')
					or ' class="text-lg font-semibold"'
			end
		...
		if stats.TotalTitles then
			statCard(stats.TotalTitles, "titles")
		end
		if stats.StatusLine and stats.StatusLine ~= "" then
			statCard(stats.StatusLine, "status", nil, true)
		end
		if stats.ReadLine and stats.ReadLine ~= "" then
			statCard(stats.ReadLine, "read", nil, true)
		end
		if stats.HasUpdates and stats.HasUpdates > 0 then
			statCard(stats.HasUpdates, "updates", "text-indigo-400")
		end
		if stats.ReadingTime and stats.ReadingTime ~= "" then
			statCard(stats.ReadingTime, "reading time", nil, true)
		end
		if stats.MostLine and stats.MostLine ~= "" then
			statCard(stats.MostLine, "most chapters", nil, true)
		end
		if stats.HasFewest and stats.FewestLine and stats.FewestLine ~= "" then
			statCard(stats.FewestLine, "fewest chapters", nil, true)
		end
		...
		emit(
			'                <span>Duplicate <span class="text-indigo-400 font-semibold">'
				.. h(tostring(duplicateCount))
				.. "</span></span>"
		)
		emit(
		...
		if duplicateCount == 0 then
			emit(
				'                <div class="text-xs text-neutral-500">Tidak ada judul yang berpotensi duplikat.</div>'
			)
		end
		for _, g in ipairs(duplicateGroups) do
			...
			for _, m in ipairs(g.Members or {}) do
				...
				if m.CoverURL and m.CoverURL ~= "" then
					...
					emit(
						'                            <div class="w-8 h-11 rounded bg-neutral-800 flex items-center justify-center text-[10px] text-neutral-500 font-semibold shrink-0">'
	...
	if #mangas == 0 then
		...
		for _, m in ipairs(mangas) do
			...
			if coverURL ~= "" then
				...
				emit(
					'            <div class="w-full aspect-[2/3] bg-neutral-800 flex items-center justify-center text-neutral-500 text-2xl font-semibold">'
						.. h(getInitials(title))
						.. "</div>"
				)
			end
			...
			if status ~= "" then
				emit(
					'        <span class="absolute top-2 left-2 bg-black/60 backdrop-blur rounded-full px-2 py-0.5 text-[10px] text-neutral-200">'
						.. h(status)
						.. "</span>"
				)
			end
			...
			if statsObj then
				...
				if pluginIcon ~= "" then
					emit(
						'<img src="'
							.. h(pluginIcon)
							.. '" alt="" class="inline h-2.5 w-2.5 rounded-sm object-cover align-[-0.5px]"> '
					)
				end
				emit(h(pluginName) .. "</span>")
				if hasNew then
					emit(
						'                <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-red-500 text-white font-medium">New</span>'
					)
				end
				emit("            </div>")
			end

...
return function(data)
	...
	if #plugins == 0 then
		...
		for _, p in ipairs(plugins) do
			...
			if iconURL ~= "" then
				...
				emit(
					'    <div class="h-10 w-10 rounded-md bg-neutral-800 flex items-center justify-center text-sm font-semibold text-neutral-400 shrink-0">'
						.. h(firstChar)
						.. "</div>"
				)
			end
			...
			if pinnedProfile ~= "" then
				emit(
					'        <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-sky-500/15 text-sky-400">'
						.. h(pinnedProfile)
						.. "</span>"
				)
			end
			...
			if siteURL ~= "" then
				emit(
					'      <a href="'
						.. h(siteURL)
						.. '" target="_blank" rel="noopener" class="text-xs text-indigo-400 hover:text-indigo-300 truncate">'
						.. h(siteURL)
						.. "</a>"
				)
			end
			...
			if needsHumanVerify then
				...
				if #verifyURL > 0 then
					emit(
						'  <a href="'
							.. h(verifyURL)
							.. '" target="_blank" rel="noopener" class="inline-block bg-indigo-600 hover:bg-indigo-500 text-white rounded-md px-3 py-1.5 text-sm font-medium mb-3">Open verification page</a>'
					)
				end
	...
	emit("      setBtnState(btn, d.ok ? 'ok' : 'fail');")
	emit("      if (d.ok) {")
	...
	emit("    })")
	emit("    .catch(function() {")
	emit("      setBtnState(btn, 'fail');")
	emit("      showToast('Test request failed', true);")
	emit("    });")
	emit("}")
	emit("")
	emit("function setBtnState(btn, state) {")
	emit("  btn.disabled = (state === 'busy');")
	emit("  var s = btn.style;")
	emit(
	...
	emit("  if (state === 'busy') {")
	emit("    s.background = '#d97706'; s.color = '#fff'; s.borderColor = '#d97706';")
	emit("    btn.textContent = 'Testing…';")
	emit("  } else if (state === 'ok') {")
	emit("    s.background = '#059669'; s.color = '#fff'; s.borderColor = '#059669';")
	emit("    btn.textContent = 'OK ✓';")
	emit("  } else {")
	emit("    s.background = '#dc2626'; s.color = '#fff'; s.borderColor = '#dc2626';")
	emit("    btn.textContent = 'Blocked ✗';")
	emit("  }")
	emit("}")
	emit("")

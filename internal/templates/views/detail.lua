...
return function(data)
	...
	if manga.CoverURL and manga.CoverURL ~= "" then
		...
		emit(
			'        <div class="w-full aspect-[2/3] bg-neutral-800 rounded-lg flex items-center justify-center text-neutral-500 text-4xl font-semibold">'
	...
	if manga.Status and manga.Status ~= "" then
		emit(
			'            <span class="text-xs px-2 py-0.5 rounded-full bg-neutral-700/50 text-neutral-400">'
				.. h(manga.Status)
				.. "</span>"
		)
	end
	...
	if #altTitles > 0 then
		...
		for _, t in ipairs(altTitles) do
			...
			if t.Source and t.Source ~= "" then
				emit(
					'                    <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-neutral-700/50 text-neutral-400">via '
						.. h(t.Source)
						.. "</span>"
				)
			end
	...
	if #altTitleServers > 0 then
		emit(
			'            <form method="post" action="/action/fetch-alt-titles/'
				.. h(pluginID)
				.. "/"
				.. h(mangaID)
	...
	if #altSummaryServers > 0 then
		emit(
			'            <form method="post" action="/action/fetch-alt-summaries/'
				.. h(pluginID)
				.. "/"
				.. h(mangaID)
	...
	if inLibrary then
		emit(
			'                <button type="submit" class="border border-emerald-600/50 text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-md px-4 py-2 text-sm">✓ In Library</button>'
		)
	else
		emit(
			'                <button type="submit" class="bg-indigo-600 hover:bg-indigo-500 text-white rounded-md px-4 py-2 text-sm font-medium">+ Add to Library</button>'
		)
	end
	emit("            </form>")

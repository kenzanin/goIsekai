...
return function(data)
	...
	if manga.CoverURL and manga.CoverURL ~= "" then
		coverHTML = '<div id="cover-wrap" class="relative" data-key="' .. h(pluginID) .. ':' .. h(mangaID) .. '">'
			.. '<img src="/image?pluginID=' .. h(pluginID) .. '&amp;url=' .. h(manga.CoverURL)
			.. '" alt="' .. h(manga.Title or "")
	...
	body = body .. coverHTML .. '</div>'
		.. '<div class="flex-1 min-w-0">'
		.. '<h1 class="text-2xl font-semibold mb-2">' .. h(manga.Title or "") .. '</h1>'

	-- Plugin badge + status + genres
	local genresHTML = ""
	local genres = manga.Genres or {}
	for _, g in ipairs(genres) do
		genresHTML = genresHTML .. '<span class="text-xs px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-400">' .. h(g) .. '</span>'
	end
	...
	if #altTitles > 0 then
		...
		atBody = atBody
			...
			.. '<span class="text-indigo-300" title="Current main title">' .. h(currentTitle) .. '</span>'
			.. '<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-indigo-900/40 text-indigo-300">via ' .. h(pluginID) .. '</span>'
			.. '</span>'
		for _, t in ipairs(altTitles) do
			atBody = atBody
				...
				.. '<form method="post" action="/action/set-title/' .. h(pluginID) .. '/' .. h(mangaID) .. '" class="inline">'
				.. '<input type="hidden" name="title" value="' .. h(t.Title or "") .. '">'
				.. '<button type="submit" title="Set as main title" class="text-neutral-300 hover:text-indigo-300" data-confirm="Set as main title?">' .. h(t.Title or "") .. '</button>'
				.. '</form>'
				.. (t.Source and t.Source ~= "" and ('<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-neutral-700/50 text-neutral-400">via ' .. h(t.Source) .. '</span>') or "")
				.. '<form method="post" action="/action/remove-alt-title/' .. h(pluginID) .. '/' .. h(mangaID) .. '" class="inline-flex">'
				.. '<input type="hidden" name="title" value="' .. h(t.Title or "") .. '">'
	...
	body = body
		.. '<div class="mb-4">'
		.. '<button type="button" onclick="' .. chevOnclick .. '" class="flex items-center gap-1.5 cursor-pointer group select-none">'
		...
		.. '</span></button>'
		.. '<div id="alt-titles-body" class="' .. (atCount > 0 and "hidden" or "") .. '">' .. atBody .. '</div>'
		.. '</div>'
	...
	if #altTitleServers > 0 then
		...
		for _, s in ipairs(altTitleServers) do
			optionsHTML = optionsHTML .. '<option value="' .. h(s.ServerID or "") .. '">' .. h(s.Name or "") .. '</option>'
		end
		titlesFormHTML = '<form method="post" action="/action/fetch-alt-titles/' .. h(pluginID) .. '/' .. h(mangaID) .. '" class="flex flex-wrap items-center gap-2">'
			.. '<select name="server" class="bg-neutral-900 border border-neutral-700 rounded-md px-2 py-1.5 text-xs text-neutral-300 focus:outline-none focus:border-indigo-500">' .. optionsHTML .. '</select>'
		...
		titlesFormHTML = '<p class="text-xs text-neutral-500">No alt-title providers available — install a plugin that declares alt-title servers.</p>'
	end
	...
	if manga.Description and manga.Description ~= "" then
		body = body
			...
			.. '<p class="text-sm text-neutral-400 mt-1.5">' .. h(manga.Description) .. '</p></div>'
	end
	...
	if #altSummaries > 0 then
		for _, a in ipairs(altSummaries) do
			asBody = asBody
				...
				.. '<form method="post" action="/action/set-summary/' .. h(pluginID) .. '/' .. h(mangaID) .. '" class="flex-1 min-w-0 group" data-confirm="Set this as the main summary?">'
				.. '<input type="hidden" name="description" value="' .. h(a.Description or "") .. '">'
				.. '<button type="submit" title="Set as main summary" class="w-full text-left text-sm text-neutral-300 hover:text-indigo-300 transition">' .. h(a.Description or "") .. '</button>'
				.. '</form>'
				.. '<div class="flex items-center gap-1.5 shrink-0 mt-0.5">'
				.. (a.Source and a.Source ~= "" and ('<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-neutral-700/50 text-neutral-400">via ' .. h(a.Source) .. '</span>') or "")
				.. '<form method="post" action="/action/remove-alt-summary/' .. h(pluginID) .. '/' .. h(mangaID) .. '" class="inline-flex">'
				.. '<input type="hidden" name="description" value="' .. h(a.Description or "") .. '">'
	...
	body = body
		...
		.. '</span></button>'
		.. '<div id="alt-summaries-body" class="' .. (asCount > 0 and "hidden" or "") .. '">' .. asBody .. '</div></div>'
	...
	if #altSummaryServers > 0 then
		...
		summariesFormHTML = '<form method="post" action="/action/fetch-alt-summaries/' .. h(pluginID) .. '/' .. h(mangaID) .. '" class="flex flex-wrap items-center gap-2 mt-1">'
			.. '<select name="server" class="bg-neutral-900 border border-neutral-700 rounded-md px-2 py-1.5 text-xs text-neutral-300 focus:outline-none focus:border-indigo-500">' .. sOptions .. '</select>'
			.. '<button type="submit" class="border border-neutral-700 hover:bg-neutral-800 rounded-md px-3 py-1.5 text-xs font-medium inline-flex items-center gap-1.5">'
			.. '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-3.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>'
			.. ' Get alternative summaries</button></form>'
	else
		summariesFormHTML = '<p class="text-xs text-neutral-500">No alt-summary providers available — install a plugin that declares alt-summary servers.</p>'
	end
	...
	local actionsHTML = '<div class="flex flex-wrap items-center gap-2 mb-4">'
		.. '<form method="post" action="/action/toggle-library/' .. h(pluginID) .. '/' .. h(mangaID) .. '">'
		.. (inLibrary
			and '<button type="submit" class="border border-emerald-600/50 text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-md px-4 py-2 text-sm">✓ In Library</button>'
			or '<button type="submit" class="bg-indigo-600 hover:bg-indigo-500 text-white rounded-md px-4 py-2 text-sm font-medium">+ Add to Library</button>')
		.. '</form>'
	...
	if continuePoint then
		...
		actionsHTML = actionsHTML
			.. '<a href="/view/read/' .. h(pluginID) .. '/' .. h(mangaID) .. '/' .. h(cp.ChapterID or "") .. '?page=' .. tostring(cp.Page or 1)
			.. '" class="bg-indigo-600 hover:bg-indigo-500 text-white rounded-md px-4 py-2 text-sm font-medium" title="' .. h(titleAttr) .. '">'
			.. h(label)
			.. '</a>'
	end
	...
	if #chapters == 0 then
		...
		body = body
			...
			.. '<input type="hidden" name="pluginID" value="' .. h(pluginID) .. '">'
			.. '<input type="hidden" name="mangaID" value="' .. h(mangaID) .. '">'
			...
			.. '</form>'
			.. '<form method="post" action="/action/reset-progress-all/' .. h(pluginID) .. '/' .. h(mangaID) .. '">'
			...
			.. '</form>'
			.. '<form method="post" action="/action/clear-cache/' .. h(pluginID) .. '/' .. h(mangaID) .. '" data-confirm="Clear cached images for this manga?">'
		...
		for i, c in ipairs(chapters) do
			...
			local rowHTML = '<div class="flex items-center gap-3 py-3 px-2 -mx-2 rounded hover:bg-neutral-900 transition">'
				.. '<input type="checkbox" name="chapterIDs" value="' .. h(cID) .. '" form="mark-bulk" class="size-4 accent-indigo-600 shrink-0">'
				.. '<a href="/view/read/' .. h(pluginID) .. '/' .. h(mangaID) .. '/' .. h(cID) .. '" class="flex-1 flex items-center justify-between gap-4 min-w-0">'
				.. '<span class="text-sm">'
				.. '<span class="text-neutral-400 mr-2">Ch. ' .. h(formatChapterNum(chapterNum)) .. '</span>'
				.. (isDone and ('<span class="line-through text-neutral-500">' .. h(cTitle) .. '</span>') or (' ' .. h(cTitle)))
				.. '</span>'
			...
			if totalPages > 0 then
				rowHTML = rowHTML
					...
					.. h(tostring(lastPageRead)) .. '/' .. h(tostring(totalPages))
					.. ' read'
			...
			elseif cachedPages > 0 then
				rowHTML = rowHTML
					.. '<span class="text-xs px-2 py-0.5 rounded-full bg-neutral-800 text-neutral-400">'
					.. h(tostring(cachedPages)) .. ' cached</span>'
			end
			...
			if c.ReleasedAt then
				...
				if rawDate ~= "" and string.sub(rawDate, 1, 4) ~= "0001" then
					rowHTML = rowHTML
						.. '<span class="text-xs text-neutral-500">' .. h(formatDate(rawDate)) .. '</span>'
				end
			end
			...
			-- Mark read button
			rowHTML = rowHTML
				.. '<form method="post" action="/action/mark-read/' .. h(pluginID) .. '/' .. h(mangaID) .. '/' .. h(cID) .. '">'
			...
			rowHTML = rowHTML
				.. '<form method="post" action="/action/reset-progress/' .. h(pluginID) .. '/' .. h(mangaID) .. '/' .. h(cID) .. '">'
			...
			-- Mark range button
			rowHTML = rowHTML
				.. '<form method="post" action="/action/mark-read-range/' .. h(pluginID) .. '/' .. h(mangaID) .. '/' .. h(firstID) .. '/' .. h(cID) .. '">'
			...
			-- CBZ download button
			rowHTML = rowHTML
				.. '<form method="post" action="/action/export-cbz/' .. h(pluginID) .. '/' .. h(mangaID) .. '/' .. h(cID) .. '">'
				.. '<input type="hidden" name="title" value="' .. h(manga.Title or "") .. ' - Ch. ' .. h(formatChapterNum(chapterNum)) .. '">'

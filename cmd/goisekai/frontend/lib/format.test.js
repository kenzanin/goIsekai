...
}
{
  ...
  assert(a instanceof Uint8Array && a[0] === 1 && a[2] === 3 && a.length === 3, "toBytes(array) -> Uint8Array");
}
{
  const a = toBytes('');
...
assert(detectImageType(new Uint8Array([0x00, 0x00, 0x00, 0x00])) === 'image/jpeg', "unknown -> jpeg default");
...
{
  const plugins = [{ ID: 'mangadex', Name: 'MangaDex' }, { ID: 'dummy', Name: 'Dummy' }];
...
assert(lookupPluginName([{ ID: 'noid' }], 'noid') === 'noid', "lookupPluginName missing Name -> ID fallback");
...
assert(formatChapterNum('7.20') === '7.2', "formatChapterNum '7.20' -> '7.2' (trailing zero stripped)");

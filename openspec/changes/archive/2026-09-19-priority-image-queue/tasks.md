...
- [x] 1.1 Add a `Prio` type (constants `PrioLow`, `PrioHigh`) and replace
...
- [x] 1.2 Change `GetImage` to take a `Prio` parameter and acquire the
...
- [x] 1.3 Add a unit test proving ordering: with a host saturated by low
...
- [x] 2.1 Parse `?prio=` in `internal/httpserver/image.go`: `high` →
      high, anything else → low; forward to `GetImage`. Verify:
      `go vet`/`just check` clean.
- [x] 2.2 Extend the httptest-level test (or add one) covering absent,
...
- [x] 3.1 `cmd/goisekai/frontend/lib/reader.js`: add a `prio` argument to
...
- [x] 3.2 Templates: detail hero cover sends `prio=high`
...
- [x] 4.1 Run `just test` and `just check`; both clean.
- [x] 4.2 Live check: start the server, open a search with many covers,

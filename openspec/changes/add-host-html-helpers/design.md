...
**The handle has no explicit release.** It is an ordinary GC-able value in each VM: the Lua and JS closures capture the Go handle, so the tree dies when the plugin drops its last reference, and a whole VM is discarded on unload or reload. A `doc.close()`-style API was rejected because the repo has no host-side resource registry to register the handle with, and a plugin that forgets to close would hold the tree for the life of the instance — strictly worse than letting the runtime's collector do it.
...
- **The handle keeps a DOM tree alive, where per-call parsing would not.** A plugin that parses inside a loop holds one tree per iteration until the VM collectes them. → Mitigation: the tree is released by ordinary garbage collection once the plugin drops the reference; the per-invocation timeout bounds how many pages one call can parse, and the per-instance memory cap bounds the total.
